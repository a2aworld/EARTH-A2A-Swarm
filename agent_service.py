...
from config import GEMINI_API_KEY, A2A_SECRET_KEY, AGENT_CARDS_DIR, GEMINI_FLASH_MODEL
...
# Load Identity
CARD_PATH = AGENT_CARDS_DIR / f"{AGENT_NAME}_card.json"
...
def verify_token(credentials = Depends(security)):
    try:
        payload = jwt.decode(
            ...
            SECRET_KEY,
            algorithms=["HS256"],
            audience=f"a2a-agent-{AGENT_NAME.replace('_', ' ')}",
            issuer="earth-orchestrator"
        )
    except:
